...
describe("rules sandbox", () => {
  it("settles a main pot and multiple side pots by eligibility", () => {
    ...
    expect(
      result.players.reduce((sum, player) => sum + player.awarded, 0),
    ).toBe(600);
  });
  ...
  it("splits a tied odd chip according to button order", () => {
    ...
    expect(result.players.find((player) => player.id === "p3")?.awarded).toBe(
      0,
    );
  });
  ...
  it("rejects duplicate cards and illegal action amounts", () => {
    ...
    expect(result.errors.some((error) => error.includes("牌面重复"))).toBe(
      true,
    );
