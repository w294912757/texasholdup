...
function validateCards(board: Card[], players: SandboxPlayerInput[]): string[] {
  ...
  const duplicates = cards.filter(
    (card, index) => cards.indexOf(card) !== index,
  );
...
function validateActions(
  ...
  actions.forEach((entry, index) => {
    ...
    if (
      (entry.action === "fold" || entry.action === "check") &&
      entry.amount !== undefined
    )
      errors.push(`${prefix}：弃牌或过牌不应填写金额`);
    if (
      entry.amount !== undefined &&
      entry.amount > player.stack + player.committedHand
    )
...
export function evaluateSandboxCase(input: SandboxCase): SandboxEvaluation {
  ...
  for (const player of players) {
    if (player.holeCards.length !== 2)
      errors.push(`${player.name} 必须有 2 张底牌`);
  ...
  buildPots(players).forEach((pot, index) => {
    ...
    const winnerIds = pot.eligiblePlayerIds.filter((id) =>
      winners.includes(solved.get(id)!),
    );
    const winnerOrder = orderedIds(players, input.dealerSeat).filter((id) =>
