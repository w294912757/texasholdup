...
test("runs an isolated rules sandbox case", async ({ page }) => {
  ...
  await expect(page.locator(".sandbox-header__title")).toContainText("测试沙盒");
...
test("keeps sandbox usable in a narrow portrait viewport", async ({ page }) => {
  ...
  const overflow = await page.evaluate(() => {
    const offenders = Array.from(document.querySelectorAll<HTMLElement>("*"))
      ...
      .map((element) => `${element.className}:${element.getBoundingClientRect().right}`);
    return { value: document.documentElement.scrollWidth > window.innerWidth + 1, offenders: offenders.slice(0, 12) };
  });
